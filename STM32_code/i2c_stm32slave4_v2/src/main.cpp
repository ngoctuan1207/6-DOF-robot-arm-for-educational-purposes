...
const float J4_H = 121.0;
const float J4_L =-121.0;
const float J4_EH = 100.01/100;
const float J4_EL = 99.99/100;
...
const uint8_t address_slave = address_slave_arr[4];
...
static bool moveOffs_Issued = false;
volatile bool isCalib = false;
unsigned int Pulse_Per_Revolution_PPR = 10000;            //Constant number
const float pulley_ratio = 0.454;                             //No pulley 
const float Gearbox_4_Ratio = 0.13;                     //Gearbox 1/47.2
const float ratio_total = pulley_ratio*Gearbox_4_Ratio;
...
void CLEAR_POSITION();
void SET_POS_INIT(float theta4_init);
void SET_ACCELERATION_TIME(unsigned int accel_time);
void SET_DECELERATION_TIME(unsigned int decel_time);
void SAVE_PARAMETER();
unsigned int SPEED_SERVO_4(float degree_target, float degree_target_old, float time_target);
float GET_SPEED_4();
float GET_THETA_4();
bool GET_MOVING_FLAG_SERVO_4();
...
bool I2C_parsing(){
  ...
  if (numByte != I2CSize) {
    ...
    if (cmd_flag != Standby){
      ...
      //   // delayMicroseconds(Process_delay);
      // }
      // Serial.printf("I2C_parsing J4: targetDeg_Int = %.2f, cmd_flag = %d,\n J4 targetVel = %.2f, targetAccel = %.2f \n\n", targetDeg_Int, cmd_flag, targetVel,targetAccel);
    } 
    ...
    switch(cmd_flag){
      case Angle:{
        ...
        // if (move_Issued == false && (isTarget_inLimit(targetDeg_Int) && isTargetOld(targetDeg_Int))){
          // Serial.printf("J4_Angle_tar = %.2f \n",targetDeg_Int);
          stat_flag = Busy;
          SERVO_MoveSingleAxisAbsPos(ID_4,DEG_TO_POS_4_CONV(targetDeg_Int),targetVel);
          delayMicroseconds(Process_delay);
          move_Issued = true;
        }
        cmd_flag = 0;
      ...
      case Move: {
        if(isTarget_inLimit(targetDeg_Int) && isTargetOld(targetDeg_Int)){
          stat_flag = Busy;
          // Serial.printf("Move J4 to abs %.2f \n",targetDeg_Int);
          SERVO_MoveSingleAxisAbsPos(ID_4,DEG_TO_POS_4_CONV(targetDeg_Int),80000);
          delayMicroseconds(Process_delay);
          move_Issued = true;
        }
        cmd_flag = 0;
      ...
      case Calib:{
        ...
        Measure_angle_int();
        delayMicroseconds(Process_delay);
        // Serial.printf("Calib:  Curr_Angle J4 =  %.2f \n",CurrAngle_int);
        cmd_flag = 0;
        stat_flag = Calibrated;
        isCalib = true;
        Timer_Encoder(true);
        // eeprom_save(0.00, 0.00, 1);
        break;
      }

      case Standby:{
        Serial.println("Standby J4");
        cmd_flag = 0;
        break;
      }
      
      case Move_Offset:{
        // if (targetDeg_Int!= 0.0){
        if (moveOffs_Issued == false && targetDeg_Int!= 0.0 && targetDeg_Int != 370){
          stat_flag = Busy;
          SERVO_MoveSingleAxisIncPos(ID_4,DEG_TO_POS_4_CONV(targetDeg_Int),4000);
          // delayMicroseconds(Process_delay);
          moveOffs_Issued = true;
          // Serial.println("End of moving offset");
        }
        cmd_flag = 0;
        break;
      }
      ...
      case MoveHome:{
        if (isCalib == true){
          Serial.println("J4 Move home");
          if (move_Issued == false){
            stat_flag = Busy;
            SERVO_MoveSingleAxisAbsPos(ID_4,DEG_TO_POS_4_CONV(0),30000);
            delayMicroseconds(Process_delay);
            move_Issued = true;
          }
          else{
            SERVO_PositionAbsOverride(ID_4,DEG_TO_POS_4_CONV(0));
            SERVO_VelocityOverride(ID_4,30000);
          }
        }
        cmd_flag = 0;
        break;
      ...
      case Linear:{
        if (isTarget_inLimit(targetDeg_Int) && move_Issued == false){
          // Serial.printf("J4_linearStart: targetDeg_Int = %.2f, J4_targetVel = %ld \n\n", targetDeg_Int, targetVel);
          stat_flag = Busy;
          pending_linearTarget = DEG_TO_POS_4_CONV(targetDeg_Int);
          SERVO_MoveSingleAxisAbsPos(ID_4,pending_linearTarget,targetVel);
          // delayMicroseconds(Process_delay);
          move_Issued = true;
        }
        else if (isTarget_inLimit(targetDeg_Int) && move_Issued == true){
          // stat_flag = Busy;
          // Serial.printf("J4_linear: targetDeg_Int = %.2f, J4_targetVel = %ld \n\n", targetDeg_Int, targetVel);
          pending_linearTarget = DEG_TO_POS_4_CONV(targetDeg_Int);
          SERVO_VelocityOverride(ID_4,targetVel);
          // delayMicroseconds(Process_delay);
          SERVO_PositionAbsOverride(ID_4,pending_linearTarget);
          // delayMicroseconds(Process_delay);
          // move_Issued = true;
        }
        cmd_flag = 0;
...
unsigned int SPEED_SERVO_4(float degree_target, float degree_target_old, float time_target)
{
    ...
    long pulse_Difference = abs(DEG_TO_POS_4_CONV(degree_target) - DEG_TO_POS_4_CONV(degree_target_old));
...
float GET_SPEED_4()
{
    SERVO_GetActualVel(ID_4, &VEL_SERVO_4);                                                             // pps
    float angular_Velocity = (float)(VEL_SERVO_4 * 360) / (Pulse_Per_Revolution_PPR / ratio_total); // dps = (pps*360)/PPR_Total
...
{
  long Curr_Pos = 0;
  SERVO_GetActualPos(ID_4, &Curr_Pos);
...
void MOVE_ORIGIN_SERVO_4()
{
  do
  {
    SERVO_MoveVelocity(ID_4, homeSpeed * 2, CW);
  ...
  do
  {
    SERVO_MoveVelocity(ID_4, homeSpeed * 2, CCW);
  ...
  SERVO_MoveSingleAxisIncPos(ID_4, (long)(round((abs(CW_Pos - CCW_Pos))/2)), (unsigned long)homeSpeed * 2);
...
bool GET_MOVING_FLAG_SERVO_4(){
  ...
  SERVO_GetAxisStatus(ID_4, &AxisStatus);
...
void loop() {
  ...
  if (moveOffs_Issued == true){
    if (Measure_flag == 1) {
      Measure_angle_int();
      delayMicroseconds(100);
    }
    movingFlag = GET_MOVING_FLAG_SERVO_4();
    delayMicroseconds(100);
    if (movingFlag == 0){
      Measure_angle_int();
      delayMicroseconds(100);
  ...
  if (move_Issued == true){
    ...
    if (movingFlag == 0 && isRunCounter == 0){
      isRunCounter++;
    }
