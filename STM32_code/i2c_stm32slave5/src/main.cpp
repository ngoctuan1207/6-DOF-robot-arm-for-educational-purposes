...
const float J5_H = 131.0;
const float J5_L =-121.0;
const float J5_EH = 100.1/100;
const float J5_EL = 99.9/100;
const float offset_Angle = 1.0;
float offset_Zero;
const uint8_t I2CSize = 30;

const float pulley_ratio = 0.48;
const float gearBox_ratio = 0.02;
...
const uint8_t address_slave = address_slave_arr[5];
...
void setup() {
  ...
  Timer_Encoder(true);
  Serial.printf("J5 ON, Home stat = %d \n",IPS);
}
...
/*bool I2C_parsing(){
  ...
  if (numByte != I2CSize) {
    ...
    if (cmd_flag != Standby){
      ...
      else AC_stepper.setMaxSpeed(maxVel/2);
      Serial.printf("I2C_parsing: targetDeg_Int = %.2f, cmd_flag = %d, targetVel = %.2f \n", targetDeg_Int, cmd_flag, targetVel);
    } 
    // else if (cmd_flag == Standby) Serial.printf("I2C_parsing: Standby, cmd_flag = %d \n", cmd_flag);
...
bool I2C_parsing(){
  ...
  if (numByte != I2CSize) {
    ...
    if (cmd_flag != Standby){
      ...
      // Serial.printf("I2C_parsing J5: targetDeg_Int = %.2f, cmd_flag = 0x%0x,\n J5 targetVel = %.2f, targetAccel = %.2f \n\n", targetDeg_Int, cmd_flag, targetVel,targetAccel);
    } 
    RxBuff_reset();
    rxReady = false;
    return out;
...
void setup_AS5600(){
  ...
  checkMagnet_n_Connection(); 
  as5600_1.setOffset(offset);
  as5600_1.setDirection(0);   //Degree increase when turn CW
  as5600_1.setMaxAngle(4095);
  as5600_1.setPowerMode(AS5600_POWERMODE_NOMINAL);
  as5600_1.setHysteresis(3);
  as5600_1.setSlowFilter(0);
...
void Motion_Control(){
  if(I2C_parsing()== true){
    switch(cmd_flag){
      ...
      case Linear:{
        if (isTarget_inLimit(-targetDeg_Int)){
          // Serial.printf("J5_linear_tar = %.2f \n",-targetDeg_Int);
          stat_flag = Busy;
      ...
      case Move_Offset:{
        ...
        // if (move_Issued == false && targetDeg_Int!= 0.0){
          // Serial.printf("J5_offset_tar %.2f \n", targetDeg_Int);
          stat_flag = Busy;
          AC_stepper.setAcceleration(2000); 
